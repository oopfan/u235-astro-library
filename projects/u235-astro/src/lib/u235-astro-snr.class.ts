...
export class U235AstroSNR {
    ...
    init() {
        ...
        const imageResolution = ([pixelSize, binning, focalLength]: number[]): number => {
            return 206.3 * pixelSize * binning / focalLength;
        }
        
        const electronsPerSecond = ([clearApertureEquivalent, fluxAttenuation, brightness, pixelSurface, quantumEfficiency]: number[]): number => {
            return clearApertureEquivalent * mag0flux * fluxAttenuation / Math.pow(10, 0.4 * brightness) * pixelSurface * quantumEfficiency / 100;
        }
        
        const totalNoise = ([readNoise, shotNoise, skyNoise, darkNoise]: number[]) : number => {
